    ...
    pokeAPI.get(`/pokemon/${id}`)
      .then(({ data }) => {
        let pokemon = {
          ...
          moves: data.moves.map((item) => item.move.name).slice(0, 4).join(', '),
...
export const catchPokemon = (pokemon, cbSuccess) => {
  return (dispatch) => {
    baseAPI.post(`/catchPokemon`, pokemon)
    .then(({data}) => {
      dispatch({
        type: CATCH_POKEMON,
        payload: pokemon
...
export const renamePokemon = (pokemon, cbSuccess) => {
  return (dispatch) => {
    baseAPI.patch(`/renamePokemon/${pokemon.id}`, pokemon)
    .then(({data}) => {
      dispatch({
        type: RENAME_POKEMON,
